/*
 * Definitions for the scull_mem_cache char module
 */
...
#ifdef SCULLMC_DEBUG
#    ifdef __KERNEL__
#	define PDEBUG(fmt, args...) printk(KERN_DEBUG "scull_mem_cache: " fmt, ## args)
#    else
	/* user-space */
...
struct scull_mc_dev {
	void **data;
	struct scull_mem_cache *next;	/* next list-item */
...
#define SCULLMC_IOCRESET	_IO(SCULL_IOC_MAGIC, 0)
